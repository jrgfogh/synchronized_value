...
private:
  ...
  mutable synchronized_value<std::unique_ptr<node_t>> head_;
  mutable synchronized_value<node_t *> tail_;
...
public:
  ...
  [[nodiscard]] auto check_invariants() const -> bool {
    update_guard<std::unique_ptr<node_t>> headGuard{head_};
    update_guard<node_t *> tailGuard{tail_};
